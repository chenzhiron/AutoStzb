...
def module_require_zhengbing():
    operate_adb_tap(zhengbing_require_click[0], zhengbing_require_click[1])


# 再次确认
def module_require_next_click():
    executeFn(reg_ocr_verify(queding_area, 2), queding)
    operate_adb_tap(zhengbing_next_click[0], zhengbing_next_click[1])


# 二级返回
def module_return_page():
    operate_adb_tap(return_page_area[0], return_page_area[1])


# 一级返回
def module_return_next_page():
    # 识别x
    operate_adb_tap(return_page_next_area[0], return_page_next_area[1])
