...
class SiegeBattles(BaseTypeImg):
    ...
    def siege_battles(self):
        ...
        for v in self.filtered_lines:
            ...
            # 名字
            r = ocr_format_val(
                np.array(self.screenshot.crop([568, v + 195, 820, v + 195 + 70]))
            ) or ocr_format_val(
                np.array(self.screenshot.crop([568, v + 195 - 70, 820, v + 195]))
            )
            current.append(r)
            ...
            # 单次战斗数量
            battles = ocr_format_val(
                np.array(
                    self.screenshot.crop([1770, v + 195 + 130, 1830, v + 195 + 200])
                )
            ) or ocr_format_val(
                np.array(
                    self.screenshot.crop([1770, v + 195 + 70, 1830, v + 195 + 130])
                )
            )
            try:
                if battles == None:
