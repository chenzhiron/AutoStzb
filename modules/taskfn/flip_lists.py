...
class FlipLists(BaseTypeImg):
    ...
    def role_lists(self):
        ...
        for v in self.filtered_lines:
            current = [0] * 3
            state = ocr_format_val(
                np.array(
                    self.screenshot.crop([815, v + 270 + 195, 1066, v + 195 + 300])
                )
            )
            print("land state: ", state)
            if state == "成功占领":
                m_name = ocr_format_val(
                    np.array(self.screenshot.crop([563, v + 195, 800, v + 195 + 100]))
                )
                current[0] = m_name
                print("prent name: ", m_name)
                # 添加土地标志判断是否是土地还是要塞
                island = ocr_format_val(
                    np.array(self.screenshot.crop([810, v + 195, 1060, v + 195 + 100]))
                )
