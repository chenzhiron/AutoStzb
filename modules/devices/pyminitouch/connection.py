...
class MNTInstaller(object):
    ...
    def __init__(self, device_id, adb):
        self._ADB = os.path.join(cwd, adb)
    ...
    def get_abi(self):
        try:
            ...
            print(self._ADB)
            command = "{} -s {} shell getprop ro.product.cpu.abi".format(self._ADB, self.device_id)
            abi = subprocess.getoutput(command)
            logger.info("device {} is {}".format(self.device_id, abi))
            return abi
        except subprocess.CalledProcessError:
            logger.error("get device {} abi failed".format(self.device_id))
    ...
    def download_target_mnt(self):
        ...
        file_path = os.path.join(dir_path, 'libs', '{}', 'minitouch').format(self.abi)

        # push and grant
        subprocess.check_call(
            [self._ADB, "-s", self.device_id, "push", file_path, config.MNT_HOME]
        )
        subprocess.check_call(
